...
1. Collect required runtime settings:
   ...
   - Profile: optional named provider from `.env` via `--profile <name>` or `IMAGEGEN_PROFILE`.
   - API key: from the selected profile, `.env`, `IMAGEGEN_API_KEY`, `OPENAI_API_KEY`, or explicit `--api-key`.
   - Model: from the selected profile, `.env`, `IMAGEGEN_MODEL`, or explicit `--model`.
   - Base URL: from the selected profile, optional `.env`, `IMAGEGEN_BASE_URL`, `OPENAI_BASE_URL`, or `--base-url`.
   - Output path: optional `--output`; defaults to `./outputs/` under the current working directory.
...
For named profiles, pass `--profile`:
...
Required:
- One of:
  - `IMAGEGEN_PROFILE_<NAME>_API_KEY` and `IMAGEGEN_PROFILE_<NAME>_MODEL`, selected with `--profile <name>` or `IMAGEGEN_PROFILE=<name>`.
  - `IMAGEGEN_API_KEY` or `OPENAI_API_KEY`, plus `IMAGEGEN_MODEL`.
...
- `IMAGEGEN_PROFILE_<NAME>_BASE_URL`; overrides the generic base URL for that profile.
- `IMAGEGEN_PROFILE_<NAME>_ENDPOINT`; overrides the image-generation endpoint for that profile.
- `--profile`, to select one configured provider/key pair for this call.
- `--list-profiles`, to inspect configured profile names without printing secrets.
- `--size`, default `1024x1024`.
- `--output`, default auto-generates a PNG under `./outputs/` in the current working directory.
...
This skill no longer maintains a skill-local image history. If `--output` is omitted, the script writes the generated file under `./outputs/` in the current working directory. If `--output` is provided, the script uses that path as given.
